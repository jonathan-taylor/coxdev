...
Let 
$$
E_i(\xi) = \sum_{j \geq i} \xi = \sum_{j: t_j \geq t_i} \xi
$$
and
$$
S_i(\xi) = \sum_{j: s_j \geq t_i} \xi.
$$
...
$$
\bar{w}_i = \frac{\sum_{j:C_j=C_i} w_j}{\sum_{j:C_j=C_i} 1}.
$$
{\bf Again, by the form of the likelihood it only matters that $\bar{w}_i$ is corrected "correctly" for individuals
who fail $(d_i=1)$.}

We'll take the clustering that assigns all individuals
to an individual cluster {\em unless} $d_i=1$. If it is an individual that failed, we find all other individuals who failed at that time and assing them to the same cluster. In this way, for each $i$, there is a ${\tt first}(i)$ which is the
...
than in formulae: for a set of indiced which is a cluster of size $K$ (so a set of $K$ failures at some particular $t$) the corresponding entries of $\sigma$ are $\{0,1/K,2/K,\dots,(K-1)/K\}.$ 

We can now define our risk sums
$$
\begin{aligned}
\Delta_i(\xi) &= \sum_{j:C_j=C_i} \xi_j  \\
\bar{R}_i(\xi) &= \sum_{j: t_j \geq t_{i}} \xi_j - 
\sum_{m: s_m \geq t_i} \xi_m
...
It should also be clear that summing $\bar{R}_i(we^{\eta})$ over $i$ doesn't 
depend on our initial ordering based on the $t_i$'s. Similarly
summing any $\zeta_i \bar{R}_i(we^{\eta})$ with $\zeta_i$ exchangable across
failures will also not depend on our initial ordering. Finally,
it should be clear that $\Delta_i$ can be expressed in terms of the cumsums of $(w_j \zeta_j)_{1 \leq j \leq n}$.
...
$$
\left(\frac{d_i \bar{w}_i}{R_i(we^{\eta})^2}, \frac{d_i \bar{w}_i \sigma_i}{R_i(we^{\eta})^2}, \frac{d_i \bar{w}_i \sigma^2_i}{R_i(we^{\eta})^2}\right)_{1 \leq i \leq n}.
$$
